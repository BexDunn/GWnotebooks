...
#added crs, crswkt, affine to main function def
def main(epoch, lon_range, lat_range, year_range, mnt_range, poly, rf_range_low, rf_range_high, season, ls7fl, debug,crs, crswkt, affine):
    ...
    #bex added crs, crswkt, affine
    cr_info = MyCrop(dc, lon_range, lat_range, products, epoch, year_range, mnt_range, poly, rf_range_low, rf_range_high, season, ls7fl, debug,crs, crswkt, affine)
...
#bex edit added crs, crswkt, affine
class MyCrop():
    def __init__(self, dc, lon_range, lat_range, products, epoch, year_range, mnt_range, poly, rf_range_low, rf_range_high, season, ls7fl, debug, crs, crswkt, affine):
        self.dc = dc
        if len(lon_range) > 0:
    ...
    #Bex edit: added returns for crs, affine, crswkt
    def build_my_dataset(self, acq_min, acq_max, dry_years, wet_years, crs, affine, crswkt):
        ...
        ##Bex edit - get projection info
        crs = nbar_data.crs
        crswkt = nbar_data_data.crs.wkt
        affine = nbar_data.affine
        return crs, crswkt, affine
    ...
    def cov_task(self, acq_min, acq_max, dry_years, wet_years):
        ...
        dry_data, wet_data  = self.build_my_dataset(acq_min, acq_max, dry_years, wet_years,crs, affine, crswkt)
