...
Created by Bex Dunn 08/05/2017. Modified 02/06/2017 to fix normalisation so it normalises per valid pixel not by timeseries as a whole. Modified 02/06/2017 to remove the error message about >- symbols and to print which shapefile is
...
##Testing the removal of the following statements. They take time to run and are for a hovmoller that we are not 
##generating when we run the code on Raijin. They also prevent dask implementation.
...
##Testing the change of the following statements. 
#wat_plus_wetv = wetness_multi.where(wetness_multi>-400)
#wet_count = wat_plus_wetv.count(dim = 'time')
#wet_count = wet_count/len(wetness_multi.time)
